...
def softmax_bias(answers, temperature=1):
    sums = 0.0
    answers = [10 ** ((answer / temperature) / 400) for answer in answers]
    for answer in answers:
        assert type(answer) == float or type(answer) == int
        sums += answer
    answers = [answer / sums for answer in answers]
...
# 大模型的流式生成接口
# TODO(@zyw): stream_interval的作用他没实现
...
def replace_llama_with_condense(ratio: int = 4): 
