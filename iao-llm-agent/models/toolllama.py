...
from models.utils import SimpleChatIO, generate_stream, react_parser
from adapters.model_adapter import get_conversation_template
from utils import process_system_message


class ToolLLaMA(BaseLLM): 

    def __init__(self, model_name_or_path: str, template: str = "tool-llama-single-round", device: str = "cuda", cpu_offloading: bool = False): 
        self.template = template
        self.tokenizer = AutoTokenizer.from_pretrained(model_name_or_path, use_fast=False, model_max_length=8192)
        self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, low_cpu_mem_usage=True)
        ...
        if (device == "cuda" and not cpu_offloading): 
    ...
    def parse(self, functions, process_id, **args): 
        conversation_template = get_conversation_template(self.template)
        if self.template == "tool-llama": 
            roles = {
                "human": conversation_template.roles[0], 
                "gpt": conversation_template.roles[1]
            }
        elif self.template == "tool-llama-single-round" or self.template == "tool-llama-multi-rounds": 
            roles = {
                "system": conversation_template.roles[0], 
                "user": conversation_template.roles[1], 
                "function": conversation_template.roles[2], 
                "assistant": conversation_template.roles[3]
            }
