...
class TreeNode: 
    ...
    def __init__(self): 
        ...
        self.description = ""
        self.observation = ""
        self.observation_code = None    # 4 for pruned
        self.children = []

        self.father = None
        
        self.env = None
        self.expand_num = 0    # 本节点的被访问次数，值为0表示尚未被访问过

        self.Elo = 1000.0

        self.messages = []    # OpenAI API格式的对话内容
    ...
    def print(self,process_id = 0):
        ...
        print(colored(f"{self.node_type}: {self.description}", color=node_type_to_color[self.node_type]))
        if self.observation != "": 
    ...
    def to_json_recursive(self, use_messages=False):
        json_obj = self.to_json(use_messages=use_messages)
        json_obj["children"] = []
        for child in self.children: 
            json_obj["children"].append(child.to_json_recursive())
        return json_obj
    ...
    def get_former_trice_from_this_node(self, valid_types=["Thought", "Action", "Action Input", "Observation"], end_node = None): 
        ...
        while node != end_node and node.father != None:
            now_node_description_list = []
            if node.node_type in valid_types:
                now_node_description_list.append(f"{node.node_type}: {node.description}\n")
            if node.observation != "" and "Observation" in valid_types:
                truncated = node.observation
                if len(node.observation) > 1024:
                    truncated = node.observation[:1024] + f"...(len={len(node.observation)})"
                now_node_description_list.append(f"Observation: {truncated}\n")
            output_str_list = now_node_description_list + output_str_list
            node = node.father
    ...
    def to_json(self, use_messages=False): 
        ...
        if self.env != None and self.node_type == "Action Input":
            json_obj["io_state"] = self.env.to_json()
